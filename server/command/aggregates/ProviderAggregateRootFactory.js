...
function AggregateRoot(){
  ...
  this.aggregateRootName = "Provider";
  ...
  this.createProvider = function(request){
    ...
    commit.action = constants.pubSub.message.provider.action.eventCommit.created;
    that.changes.push(commit);
  };

  this.deleteProvider = function(request){
    // var validationResult = await userValidator.validateCreate(request);
    //
    // if (validationResult) {
    //   return internalEventEmitter.emit(
    ...
    commit.aggregateRootName = that.aggregateRootName;
    commit.action = constants.pubSub.message.provider.action.eventCommit.deleted;
    that.changes.push(commit);
  ...
  this.addBusiness = function(request){
    ...
    commit.aggregateRootId = request.id;
    commit.aggregateRootName = that.aggregateRootName;
    commit.action = constants.pubSub.message.provider.action.eventCommit.businessAdded;
    that.changes.push(commit);
  ...
  this.removeBusiness = function(request){
    ...
    commit.aggregateRootId = request.id;
    commit.aggregateRootName = that.aggregateRootName;
    commit.action = constants.pubSub.message.provider.action.eventCommit.businessRemoved;
    that.changes.push(commit);
